    ...
    if cfg_source == 'hf-hub' and has_hf_hub(necessary=True):
        ...
        if pretrained_sd:
            ...
            if _USE_OLD_CACHE:
                ...
                old_cache_valid = check_cached_file(
                    pretrained_url) if pretrained_url else False
...
def load_pretrained(
    ...
    if load_from == 'state_dict':
        ...
        # pretrained_loc is the actual state dict for this override
        state_dict = pretrained_loc
    elif load_from == 'file':
        _logger.info(
            f'Loading pretrained weights from file ({pretrained_loc})')
    ...
    if classifiers is not None:
        ...
        if num_classes != pretrained_cfg['num_classes']:
            ...
            for classifier_name in classifiers:
                ...
                state_dict[classifier_name +
                           '.bias'] = classifier_bias[label_offset:]
...
def resolve_pretrained_cfg(
    ...
    pretrained_cfg = dataclasses.replace(
...
def build_model_with_cfg(
    ...
    if features:
        ...
        model.pretrained_cfg = pretrained_cfg_for_features(
            pretrained_cfg)  # add back pretrained cfg

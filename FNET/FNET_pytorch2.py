...
class FourierMMLayer(nn.Module):
    def __init__(self, config):
        ...
        self.dft_mat_seq = torch.tensor(
            linalg.dft(config['max_position_embeddings']))
...
class FourierFFTLayer(nn.Module):
    ...
    def forward(self, hidden_states):
        return torch.fft.fft(torch.fft.fft(hidden_states.float(), dim=-1), dim=-1).real


class FNetLayer(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.fft = FourierMMLayer(
            config) if config['fourier'] == 'matmul' else FourierFFTLayer()
...
class FNetEncoder(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.config = config
        self.layer = nn.ModuleList([FNetLayer(config)
...
class FNetForPreTraining(nn.Module):
    def __init__(self, config):
        ...
        self.mlm_intermediate = nn.Linear(
    ...
    def forward(self, input_ids, type_ids, mlm_positions=None):
        ...
        if mlm_positions is not None:
            mlm_input = sequence_output.take_along_dim(
                mlm_positions.unsqueeze(-1), dim=1)
        else:

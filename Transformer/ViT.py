...
from utils._builder import build_model_with_cfg
from utils._manipulate import named_apply, checkpoint_seq, adapt_input_conv
from utils._registry import generate_default_cfgs, register_model, register_model_deprecations
...
class ParallelScalingBlock(nn.Module):
    ...
    def __init__(
        ...
        if qkv_bias:
            ...
            self.register_buffer(
                'qkv_bias', torch.zeros(3 * dim), persistent=False)
        ...
        self.ls = LayerScale(
            dim, init_values=init_values) if init_values is not None else nn.Identity()
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        if self.mlp_bias is not None:
            ...
            y = F.linear(y, self.in_proj.weight, torch.cat(
                (self.qkv_bias, self.mlp_bias)))
        else:
            y = self.in_proj(y)
        x_mlp, q, k, v = torch.split(y, self.in_split, dim=-1)

        # Dot product attention w/ qk norm
        q = self.q_norm(q.view(B, N, self.num_heads,
                        self.head_dim)).transpose(1, 2)
        k = self.k_norm(k.view(B, N, self.num_heads,
                        self.head_dim)).transpose(1, 2)
    ...
    def __init__(
        ...
        for _ in range(num_parallel):
            self.attns.append(nn.Sequential(OrderedDict([
                ...
                ('ls', LayerScale(dim, init_values=init_values)
...
class VisionTransformer(nn.Module):
    ...
    def __init__(
        ...
        self.reg_token = nn.Parameter(torch.zeros(
            1, reg_tokens, embed_dim)) if reg_tokens else None
    ...
    def _intermediate_layers(
        ...
        take_indices = set(range(num_blocks - n, num_blocks)
...
def resize_pos_embed(
    ...
    *DEPRECATED* This function is being deprecated in favour of resample_abs_pos_embed
    ...
    posemb_grid = F.interpolate(
        posemb_grid, size=gs_new, mode=interpolation, antialias=antialias, align_corners=False)
...
def _load_weights(model: VisionTransformer, checkpoint_path: str, prefix: str = '') -> None:
    ...
    if model.attn_pool is not None:
        ...
        model.attn_pool.proj.weight.copy_(
        ...
        model.attn_pool.norm.bias.copy_(
    ...
    for i, block in enumerate(model.blocks.children()):
        ...
        block.norm2.weight.copy_(
            _n2p(w[f'{block_prefix}LayerNorm_{ln1_sub}/scale']))
        block.norm2.bias.copy_(
            _n2p(w[f'{block_prefix}LayerNorm_{ln1_sub}/bias']))
        for r in range(2):
        ...
        if k == 'proj':
            ...
            if v.shape[1] != model.pos_embed.shape[1]:
                ...
                v = resize_pos_embed(
                    v,
                    model.pos_embed,
                    0 if getattr(model, 'no_embed_class') else getattr(
                        model, 'num_prefix_tokens', 1),
...
def _convert_dinov2(
    ...
    if 'register_tokens' in state_dict:
        ...
        out_dict['cls_token'] = state_dict.pop(
            'cls_token') + state_dict['pos_embed'][:, 0]
...
def checkpoint_filter_fn(
    ...
    if prefix:
        ...
        state_dict = {k[len(prefix):]: v for k,
                      v in state_dict.items() if k.startswith(prefix)}
...
def vit_giant_patch14_224(pretrained: bool = False, **kwargs) -> VisionTransformer:
    ...
    """
    model_args = dict(patch_size=14, embed_dim=1408,
                      mlp_ratio=48/11, depth=40, num_heads=16)
    model = _create_vision_transformer(
...
def vit_gigantic_patch14_224(pretrained: bool = False, **kwargs) -> VisionTransformer:
    ...
    """
    model_args = dict(patch_size=14, embed_dim=1664,
                      mlp_ratio=64/13, depth=48, num_heads=16)
    model = _create_vision_transformer(
...
def vit_base_patch16_224_miil(pretrained: bool = False, **kwargs) -> VisionTransformer:
    ...
    """
    model_args = dict(patch_size=16, embed_dim=768,
                      depth=12, num_heads=12, qkv_bias=False)
    model = _create_vision_transformer(
...
def vit_base_patch16_clip_224(pretrained: bool = False, **kwargs) -> VisionTransformer:
    ...
    """
    model_args = dict(patch_size=16, embed_dim=768, depth=12,
                      num_heads=12, pre_norm=True, norm_layer=nn.LayerNorm)
    model = _create_vision_transformer(
...
def vit_large_patch14_clip_224(pretrained: bool = False, **kwargs) -> VisionTransformer:
    ...
    """
    model_args = dict(patch_size=14, embed_dim=1024, depth=24,
                      num_heads=16, pre_norm=True, norm_layer=nn.LayerNorm)
    model = _create_vision_transformer(
...
def vit_huge_patch14_clip_224(pretrained: bool = False, **kwargs) -> VisionTransformer:
    ...
    """
    model_args = dict(patch_size=14, embed_dim=1280, depth=32,
                      num_heads=16, pre_norm=True, norm_layer=nn.LayerNorm)
    model = _create_vision_transformer(
...
def vit_base_patch32_plus_256(pretrained: bool = False, **kwargs) -> VisionTransformer:
    """ ViT-Base (ViT-B/32+)
    """
    model_args = dict(patch_size=32, embed_dim=896, depth=12,
                      num_heads=14, init_values=1e-5)
    model = _create_vision_transformer(
...
def vit_base_patch16_plus_240(pretrained: bool = False, **kwargs) -> VisionTransformer:
    """ ViT-Base (ViT-B/16+)
    """
    model_args = dict(patch_size=16, embed_dim=896, depth=12,
                      num_heads=14, init_values=1e-5)
    model = _create_vision_transformer(
...
def vit_small_patch16_36x1_224(pretrained: bool = False, **kwargs) -> VisionTransformer:
    ...
    """
    model_args = dict(patch_size=16, embed_dim=384, depth=36,
                      num_heads=6, init_values=1e-5)
    model = _create_vision_transformer(
...
def eva_large_patch14_196(pretrained: bool = False, **kwargs) -> VisionTransformer:
    ...
    model_args = dict(patch_size=14, embed_dim=1024,
                      depth=24, num_heads=16, global_pool='avg')
    model = _create_vision_transformer(
...
def flexivit_small(pretrained: bool = False, **kwargs) -> VisionTransformer:
    """ FlexiViT-Small
    """
    model_args = dict(patch_size=16, embed_dim=384, depth=12,
                      num_heads=6, no_embed_class=True)
    model = _create_vision_transformer(
...
def flexivit_base(pretrained: bool = False, **kwargs) -> VisionTransformer:
    ...
    """
    model_args = dict(patch_size=16, embed_dim=768, depth=12,
                      num_heads=12, no_embed_class=True)
    model = _create_vision_transformer(
...
def flexivit_large(pretrained: bool = False, **kwargs) -> VisionTransformer:
    """ FlexiViT-Large
    """
    model_args = dict(patch_size=16, embed_dim=1024, depth=24,
                      num_heads=16, no_embed_class=True)
    model = _create_vision_transformer(
...
def vit_small_patch14_dinov2(pretrained: bool = False, **kwargs) -> VisionTransformer:
    """ ViT-S/14 for DINOv2
    """
    model_args = dict(patch_size=14, embed_dim=384, depth=12,
                      num_heads=6, init_values=1e-5, img_size=518)
    model = _create_vision_transformer(
...
def vit_base_patch14_dinov2(pretrained: bool = False, **kwargs) -> VisionTransformer:
    """ ViT-B/14 for DINOv2
    """
    model_args = dict(patch_size=14, embed_dim=768, depth=12,
                      num_heads=12, init_values=1e-5, img_size=518)
    model = _create_vision_transformer(
...
def vit_large_patch14_dinov2(pretrained: bool = False, **kwargs) -> VisionTransformer:
    ...
    """
    model_args = dict(patch_size=14, embed_dim=1024, depth=24,
                      num_heads=16, init_values=1e-5, img_size=518)
    model = _create_vision_transformer(
